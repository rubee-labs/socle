...
| `linked_skills` | list[str] | (optionnel) skills produits via `/skillify` à partir de ce subject. **Maintenu automatiquement** par `forge skillify scaffold --source-subjects` depuis 2026-09-14 (D10) — le skill généré porte le pointeur inverse dans son propre frontmatter (`source_subjects: [...]`). |

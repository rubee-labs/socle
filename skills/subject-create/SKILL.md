...
`forging_state` reste à `seed` (signal entrant, pas encore traité). `conviction` reste à `0`.
...
- Suites possibles :
  - `/documente <chemin>` pour re-synthétiser le subject (transition auto `seed → debating` si ≥1 event ou discussion)
...
├── MEMORY.md     (forging_state: seed, conviction: 0,
...
- **Ne pas pré-remplir trop** : `forging_state: seed` est obligatoire à la création. Ne PAS pré-remplir directement à `debating` ou `tentative` — c'est le rôle de `/documente` (qui invoque le moteur forge) de faire avancer dans le cycle.
...
- **EVAL 3** : `forging_state: seed` et `conviction: 0` sont-ils respectés à la création ? (Pass / Fail)
